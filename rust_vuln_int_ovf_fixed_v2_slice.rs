...
type EntriesType = [[u8; MAX_ENTRY_SIZE]; 32];
...
impl file::Operations for RustVuln {
    ...
    fn open(_data: &(), _file: &File) -> Result<Self::Data> {
        Ok(Pin::from(Box::try_new(RustVuln {
            entries: Mutex::new([[0; MAX_ENTRY_SIZE]; 32]),
        })?))
    }
    ...
    fn write(state: &Self, _: &File, cmd: u32, reader: &mut UserSlicePtrReader) -> Result<i32> {
        match cmd {
            VULN_COPY_ENTRIES => {
                ...
                let n_entries = entry_data.n_entries as usize;

                if n_entries > size_of::<EntriesType>() / MAX_ENTRY_SIZE {
                    pr_err!("VULN_COPY_ENTRIES: too much entry data ({})\n", n_entries * MAX_ENTRY_SIZE);
                    return Err(EINVAL);
                }

                // SAFETY: any source should be safe, since it goes through copy_from_user
                let entry_reader = unsafe { UserSlicePtr::new(entry_data.entries as _, n_entries * MAX_ENTRY_SIZE) };
                let mut entry_reader = entry_reader.reader();

                for i in 0..n_entries {
                    entry_reader.read_slice(&mut state.entries.lock()[i])?;
                }
                Ok(0)
            }
            _ => {
